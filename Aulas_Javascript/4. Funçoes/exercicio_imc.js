...
function clasificarImc(imc){
    ...
    if(imc < 18.5){
        return 'Voce tem que comer demais, esta Abaixo de peso ' + imc.toFixed(2);
    }else if (imc >= 18.5 && imc < 25){
        return 'Voce esta exelente, tem peso normal ' + imc.toFixed(2);
    }else if (imc >= 25 && imc < 30){
        return 'Voce esta comendo muito, esta acima do peso ' + imc.toFixed(2);
    }else if (imc >= 30 && imc <= 40){
        return 'Voce precisa parar de comer doce, esta obeso ' + imc.toFixed(2);
    }else {
        return 'Prepare teu ataude, voce tem obesidad grave ' + imc.toFixed(2);
    }
}
